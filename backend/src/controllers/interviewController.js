...
- All questions must be about: ${topicDescription}`;
...
export const nextQuestion = async (req, res) => {
  try {
    ...
    if (session.status !== "active") return res.status(400).json({ message: "Session is already completed" });

    if (new Date() > new Date(session.endsAt)) {
      await prisma.interviewSession.update({ where: { id: sessionId }, data: { status: "timeout" } });
    ...
    answers.push({
      questionId, question: currentQuestion.question, type: currentQuestion.type,
      ...
      explanation: currentQuestion.explanation, isCorrect, timeTaken: timeTaken || 0
    });
    ...
    if (isLastQuestion) {
      return res.json({ success: true, message: "Interview completed! Call /finish to get your analysis.", sessionId, isCompleted: true, totalAnswered: answers.length });
    }
...
export const finishInterview = async (req, res) => {
  try {
    ...
    const answers = JSON.parse(session.answers);
    if (answers.length === 0) return res.status(400).json({ message: "No answers found to analyze" });

    await prisma.interviewSession.update({ where: { id: sessionId }, data: { status: "completed", completedAt: new Date() } });

    const answersSummary = answers.map((a, i) =>
      `--- Question ${i + 1} ---
Type: ${a.type.toUpperCase()} | Topic: ${a.topic} | Difficulty: ${a.difficulty}
Question: ${a.question}
User's Answer: ${a.userAnswer}
Expected Answer: ${a.correctAnswer}
${a.isCorrect !== null ? `MCQ Result: ${a.isCorrect ? "CORRECT" : "WRONG"}` : ""}
Time Taken: ${a.timeTaken || 0} seconds`
    ).join("\n\n");

    const analysisPrompt = `You are a senior technical interviewer giving a thorough post-interview debrief.

Interview: Topic: ${TOPICS[session.topic]} | Duration: ${session.duration} min | Questions: ${answers.length}
...

{
  ...
  "questionAnalysis": [
    {
      ...
      "result": "correct",
      "accuracyPercent": 100,
      "userAnswerSummary": "what the user said",
      "whatWasRight": "specific things correct",
      "whatWasMissing": "concepts missed",
      "whatWasWrong": "incorrect statements",
      "modelAnswer": "Complete ideal answer a top candidate would give. Be thorough.",
      "conceptsToStudy": ["concept 1", "concept 2"],
      "studyResources": ["LeetCode: Two Sum", "Read about: time complexity"],
      "timeTaken": 45,
      "timeVerdict": "good"
    }
  ],
  "topicBreakdown": [
    { "topic": "Arrays", "questionsCount": 3, "correctCount": 2, "score": 80, "status": "strong", "summary": "Good grasp of basics" }
  ],
  "difficultyBreakdown": {
  ...
  "skillsAssessment": {
    "problemSolving":       { "score": 70, "feedback": "Approaches problems systematically" },
    "technicalKnowledge":   { "score": 65, "feedback": "Strong on fundamentals" },
    "codeQuality":          { "score": 60, "feedback": "Code works but lacks comments" },
    "communicationClarity": { "score": 75, "feedback": "Explains ideas clearly" },
    "timeManagement":       { "score": 80, "feedback": "Paced well" }
  },
  "strongTopics": ["Arrays", "REST APIs"],
  "weakTopics": ["Dynamic Programming", "System Design"],
  "top3Improvements": [
    "Practice DP with memoization on LeetCode",
    "Study Big-O analysis more deeply",
    "When answering design questions, cover scalability and failure handling"
  ],
  "studyPlan": {
  ...
  "readinessVerdict": "Strong junior candidate. Focus on DP and system design.",
  "estimatedLevel": "junior",
  "overallFeedback": "2-3 sentence honest overall summary"
}

RULES:
- accuracyPercent: 0-100. MCQ: 100 if correct, 0 if wrong. Text/code: estimate % of key concepts covered.
- result: "correct", "partial", or "wrong"
- modelAnswer must be complete and educational
- timeVerdict: "too fast", "good", or "too slow"
- estimatedLevel: "intern", "junior", "mid", or "senior"`;
    ...
    try {
      ...
      analysis = {
        ...
        totalQuestions: answers.length, correctAnswers: correctCount,
        wrongAnswers: mcqAnswers.length - correctCount, partialAnswers: answers.length - mcqAnswers.length,
        questionAnalysis: answers.map((a, i) => ({
          questionId: a.questionId, questionNumber: i + 1, question: a.question,
          topic: a.topic, type: a.type, difficulty: a.difficulty,
          ...
          userAnswerSummary: a.userAnswer, modelAnswer: a.correctAnswer,
          timeTaken: a.timeTaken || 0, timeVerdict: "good"
        })),
        top3Improvements: ["Review incorrect answers", "Practice more problems", "Study weak topics"],
        overallFeedback: "AI analysis failed — showing basic stats only.", error: err.message
      };
    }

    await prisma.interviewSession.update({ where: { id: sessionId }, data: { analysis: JSON.stringify(analysis) } });
...
export const getHistory = async (req, res) => {
  try {
    ...
    return res.json({ success: true, history: sessions.map(s => ({ ...s, analysis: s.analysis ? JSON.parse(s.analysis) : null })) });
  } catch (error) {
...
export const getSessionDetail = async (req, res) => {
  try {
    ...
    return res.json({
      success: true,
      session: {
        id: session.id, topic: session.topic, duration: session.duration,
        numQuestions: session.numQuestions, status: session.status,
        startedAt: session.startedAt, completedAt: session.completedAt, endsAt: session.endsAt,
